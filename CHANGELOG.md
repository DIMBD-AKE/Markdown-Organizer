...
### Fixed
- **실시간 갱신 — CPU 급등·충돌 종료(1.2.5 회귀)** — 1.2.5의 이벤트 배처가 한 묶음(batch)의 변경을 경로마다 IPC 메시지 1개씩 따로 보냈는데, 렌더러는 메시지가 올 때마다 프로젝트 전체 파일 트리를 재귀적으로 다시 스캔함. 결과적으로 파일 N개가 한 번에 바뀌면 디스크 전체 워크가 N번 동시 실행 → 큰 프로젝트(Unity 등)에서 CPU 급등·메모리 폭증·충돌 종료. flush당 메시지 1개(`{ events: [...] }`)로 묶어 트리 재빌드를 묶음당 1회로 줄임. 변경 경로 전체는 그대로 전달되므로 열린 문서 갱신 누락 없음. 1.2.5의 실시간 갱신 수정은 유지.

## [1.2.5] — 2026-06-20

### Fixed
- **실시간 갱신 — 파일 트리·문서 뷰어가 가끔 안 바뀜** — 파일 감시기의 디바운스가 이벤트가 올 때마다 200ms 타이머를 매번 리셋하고, 콜백 클로저에 마지막 이벤트의 `{type,path}` 하나만 담았음. 두 가지 누락 발생: (1) 변경이 200ms보다 촘촘히 연속으로 들어오면(Dropbox 동기화, 에디터의 다중 저장, 폴더 복사) 타이머가 끝없이 리셋돼 조용해질 때까지 한 번도 안 보냄. (2) 한 묶음(burst)에서 다른 파일 이벤트가 마지막에 오면 열려 있던 문서의 변경이 버려져 뷰어가 갱신 안 됨. 선행 엣지(leading-edge) 타이머 + 경로별 코얼레싱(`Map` 키 = 경로)으로 교체해, 연속 스트림도 첫 이벤트 후 200ms 안에 반드시 flush되고 묶음 안의 모든 고유 경로가 보존되도록 수정. 로직을 `createEventBatcher`로 분리해 단위 테스트 5종 추가.
