...
**Completed: 2026-05-27** | Release: v1.1.0 | Work-log: `docs/work-logs/2026-05-27-milestone-6-stability.md`

### 버그

**Windows**
- [x] 프로젝트 등록/오픈, 문서 클릭 시 일시적 렉
  - 수정: `READ_FILE` IPC `readFileSync` → `fs.promises.readFile` 비동기 전환
  - 수정: chokidar depth 10→5, awaitWriteFinish 추가

**macOS**
- [x] "손상된 파일" 오류 — 실행 불가
  - 수정: `afterPack` 훅에서 `codesign --force --deep --sign -` ad-hoc 서명
- [x] 트래픽 라이트(창 컨트롤) 부재
  - 수정: `titleBarStyle: 'hidden'` + `trafficLightPosition: { x:12, y:12 }` + TitleBar 80px spacer

**공통**
- [x] README 깃허브 배지 미표시 → 정적 배지로 교체, Linux deb 참조 제거
- [x] 버전 체크 404 에러 → 비공개 저장소 404/401 에러를 UPDATE_NOT_AVAILABLE로 무음 처리
...
**포터블 빌드 (설치 불필요)**
- [x] Windows: NSIS 인스톨러 + portable exe 추가 (`Markdown Organizer-Portable-1.1.0.exe`)
- [x] macOS: DMG는 이미 포터블 ✅
- [x] Linux: AppImage는 이미 포터블 ✅

**Handoff Note (2026-05-27):**
v1.1.0 완성. 핵심 수정: macOS ad-hoc 서명(afterPack hook), Windows 렉(readFileSync→async), Windows portable exe, 비공개 저장소 자동업데이트 404 무음 처리. 67/67 테스트 통과. main에 머지 완료. 다음 작업은 "이후 고려 항목" 중 선택 또는 신규 기능 기획.
...
## M10 · 윈도우 안정화 Round 4 🪟 `main` ✅

> M9 progressive streaming의 잔여 미완 + 윈도우 워처 startup 렉의 진짜 원인 + 타이틀바 테마 동기화.

**Completed: 2026-05-28** | Release: v1.1.7

### 완료 항목

**Windows 렉 — 워처 startup이 진짜 원인 (M9 가설 정정)**
- [x] 프로파일링 (`scripts/profile-tree.cjs`): 사용자 Unity 프로젝트에서 트리 walk는 67-89 ms로 빠름. 진짜 병목은 chokidar startup의 **9524 ms** — dotfile 외 무필터로 `node_modules`/`Library`/`Temp`/`obj` 전부 스캔.
- [x] `EXCLUDED_DIRS` + `UNITY_EXCLUDED` + `.md`-only를 chokidar `ignored`에 통합 → **391 ms (24배 빠름)**.
- [x] `isUnityProjectSync` — 워처 startup 시점 동기 감지 (`fs.readdirSync` 1회).
- [x] `src/main/projectFilters.ts` 공용 모듈 — 트리워크/스트리밍/워처 3곳의 상수 드리프트 방지.

**빈 폴더 prune (M9 미완 마무리)**
- [x] `pruneEmptyDirs` 재귀 헬퍼 + `completeStream()`에서 호출.
- [x] mdCount 재귀 재계산 (M9 patchChildren은 direct mdCount만 갱신).
- [x] 루트 보존 (자식이 모두 prune되어도 root는 남김 → 빈 트리 UI 정상).
- [x] 8 unit tests (단일/중첩/전체 prune/루트 보존/immutability/mdCount).

**타이틀바 테마 동기화**
- [x] `THEME_OVERLAY` 매핑 + `SET_TITLE_BAR_OVERLAY` IPC.
- [x] App.tsx `useEffect(()=>setTitleBarOverlay(theme), [theme])` — setTheme 콜러(ThemeToggle/SettingsPanel/초기 로드) 무관 자동 반영.
- [x] 윈도우 생성 시 db의 영구 저장 theme 읽어 초기 오버레이 적용 → launch flash 없음.
- [x] 오버레이 색을 `--color-base`(`#1e1e2e`)에서 헤더 실제 `bg-mantle`로 정렬 — 기존 미세 미스매치도 해소.

**검증**
- [x] 103 → 131 테스트 통과.
- [x] `tsc --noEmit` clean (node + web), `electron-vite build` clean.

**환경 fix (post-M9 working tree)**
- [x] `shiki` 직접 dependency — pnpm 격리 환경의 transitive 미해소 처리.
- [x] electron 31 바이너리 캐시 zip 수동 해제 + `path.txt` 복구 가이드 (postinstall 실패 회복 절차).

**Handoff Note (2026-05-28):**
M9는 "트리 walk가 멈춤의 원인"이라는 가설로 streaming을 도입했지만 실측에서 워처 startup이 진짜 병목임이 드러남. Streaming은 여전히 의미 있음(트리 빌드를 UI 비동기화). M10은 워처 필터링으로 startup 자체를 줄임 — 두 개선이 누적되어 대형 프로젝트 오픈 체감이 비로소 즉각으로 변환. `useFileWatcher`의 file-change → `buildFileTree` 풀 재빌드 경로는 여전히 존재하지만 워처가 `.md`만 fire하므로 트리거 빈도가 급감.
...
- 문서 그래프 (Obsidian 스타일)
- 커스텀 테마 / 사용자 CSS
- `useFileWatcher` 파일 변경 → 풀 트리 재빌드 → 점진적 patch (대형 .md 프로젝트 한정 잔존 위험)
